...
part of [`riddlg`](/riddlg/latest/), for AI-assisted model generation.
When configured:
...
[MCP-capable IDE assistant](/riddlg/latest/MCP/intellij-ai/) at `riddlg mcp`, or
run `riddlg serve` and connect to `http://127.0.0.1:8910/mcp`. No API key is
required — the server runs on your machine. See
[MCP Tools](/riddlg/latest/mcp-tools/) for the available tools.
