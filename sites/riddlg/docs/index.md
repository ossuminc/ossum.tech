...
- **Server & MCP modes** - Run as a localhost HTTP service (used by
  [Synapify](/synapify/latest/)) or as an
...
- [`riddlc`](/riddl/latest/tools/riddlc/) is the open-source compiler: validation
  only. `riddlg` includes the same validation and adds generation.
- [Synapify](/synapify/latest/) is the visual editor; it drives
  `riddlg serve` under the hood for its generation, AI, and MCP features.
- The [MCP Server](/riddl/latest/tools/riddl-mcp-server/) capabilities are also
