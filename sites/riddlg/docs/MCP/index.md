...
!!! info "The MCP server now runs locally in riddlg"
    ...
    [riddlg](../index.md) binary and run **on your
    machine** — `riddlg mcp` for stdio clients, or `POST /mcp` while
    [`riddlg serve`](../server-api.md#post-mcp) is running.
...
First, install riddlg (see [Installation](../installation.md)):
...
see [Server API](../server-api.md#post-mcp) for the HTTP
session and status-code details.

## Available Tools

Thirteen tools are exposed; see
[MCP Tools](../mcp-tools.md) for arguments and return
shapes.
...
tier. See [Free and Pro](../index.md#free-and-pro) — all
MCP tools are free.
