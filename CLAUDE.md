...
| adaptor ports | — | **DECLARED**, like every processor's (2.2.0, reversing the 2026-09-06 implied-port design). A port-less adaptor is `void`, not `flow`; two inlets and an outlet is a legal `merge`. A connector endpoint names the **portlet** (`outlet C.A.P`); naming the adaptor is `ref-wrong-kind`. `adaptor-implied-outlet-ambiguous` is retired |
| missing port | silent | **Missing warning** for EVERY processor kind (2.2.0): handlers receive but no inlet, or transmit but no outlet → "is incomplete: it handles X but declares no inlet", naming the types. Two id spellings on purpose: entity keeps `entity-no-inlet`/`-outlet`, everything else `stream-processor-no-inlet`/`-outlet`. Rules that read the missing side **abstain** — one omission, one message |
| `forward` outside an obligation | — | Error `stmt-forward-wrong-clause` — only in a clause handling a command with `yields` or a query with `replies`. A plain relay is `send <bound> to outlet X` |
| `append` / `remove` | ❌ | ✅ 2.2.0 — `append v to field F`, `remove v from field F`, `remove from field F where key == v`. Collection fields of the entity's state only; same scope/effect/event-sourcing rules as `set`. Errors: `stmt-collection-field-not-collection`, `stmt-collection-key-not-a-field`. Exist because `entity-event-sourced-prose-folds` reports a `set … to prompt("…")` list fold as prose; arithmetic stays a `prompt` on purpose |
| `Advisory` message kind | — | severity 1, **not a warning** (2.2.0): `-w` and `--fail-on warning` ignore it, never blocks generation, own switch `--show-advisories` (default true). First rule: `adaptor-direction-advisory` |
